...
const BookCard: React.FC<BookCardProps> = ({ book, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-xl transform hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="p-4">
        <Link href={`/book/${book.id}`} className="block cursor-pointer">
          <h3 className="text-lg font-semibold text-gray-800 truncate hover:text-purple-600 transition-colors duration-200">{book.title}</h3>
        ...
        <div className="mt-2">
          {book.genre.slice(0, 2).map((g) => (
            <span key={g} className="inline-block bg-purple-100 text-purple-800 rounded-full px-2 py-1 text-xs font-semibold mr-2 mb-2">{g}</span>
          ))}
          {book.genre.length > 2 && (
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          {!book.inStock && <span className="text-xs text-red-600 font-medium">Out of Stock</span>}
        </div>

        {/* Action Buttons */}
        <div className="flex gap-2 mt-4">
          <Link href={`/book/${book.id}`} className="flex-1 cursor-pointer">
            <button className="w-full px-3 py-2 text-sm border border-purple-300 text-purple-700 rounded-md hover:bg-purple-50 hover:shadow-sm transition-all duration-200 cursor-pointer">
              View Details
            </button>
          </Link>
          <button
      ...
      {/* Comment: Added useMemo for star rendering and improved hover effects & colors */}
    </div>
  );
};
